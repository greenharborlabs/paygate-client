...
APPROVAL_FIELDS = {
    "approved", "candidate_sha256", "source_commit", "cargo_lock_sha256",
    "attestation_subject", "attestation_digest", "approval_id",
    "approved_backends", "issuer", "key_id", "issued_at", "expires_at",
}
APPROVAL_ENVELOPE_FIELDS = {"claims", "signature"}
APPROVAL_KEY_FIELDS = {"id", "issuer", "public_key", "not_before", "not_after", "revoked"}
SANDBOX_POLICY_FIELDS = {
    "schema_version", "network", "inherit_environment", "mounts",
    "forbidden_mounts", "candidate_protocol",
}
PAYMENT_POLICY_FIELDS = {"schema_version", "caps_msat", "adapter_protocol"}
TRANSITIONS = {
    None: {"claimed"},
    "claimed": {"unsubmitted_released", "succeeded", "definite_failed",
                "submitted_unknown_permanent_no_retry"},
}


def attempt_key(a):
    return ":".join((a.source_commit,a.cargo_lock_sha256,a.backend,a.workflow_run_id))
...
def validate_candidate_approval(a):
    ...
    if (approval["approved"] is not True or any(not isinstance(approval[x], str) or not approval[x] for x in strings)
    ...
    if (not DIGEST.fullmatch(approval["candidate_sha256"])
    ...
    if (not isinstance(keyring, dict) or keyring.get("purpose") != "candidate-approval"
            or not isinstance(keyring.get("keys"), list)):
        raise ValueError("invalid candidate approval keyring")
    keys = [key for key in keyring["keys"]
            if isinstance(key, dict) and set(key) == APPROVAL_KEY_FIELDS
            and key["id"] == approval["key_id"] and key["issuer"] == approval["issuer"]]
    if len(keys) != 1 or keys[0]["revoked"] is not False:
        raise ValueError("unknown or revoked candidate approval key")
    key = keys[0]
    if (not isinstance(key["public_key"], str) or not key["public_key"]
            or _time(key["not_before"]) > issued or _time(key["not_after"]) < issued):
    ...
    if (approval["source_commit"] != a.source_commit
...
def load_sandbox_policy():
    ...
    if (not isinstance(policy, dict) or set(policy) != SANDBOX_POLICY_FIELDS
            or policy.get("schema_version") != 1 or policy.get("network") != "denied"
            or policy.get("inherit_environment") is not False or policy.get("mounts") != []
...
def load_payment_cap(backend):
    ...
    if (not isinstance(policy, dict) or set(policy) != PAYMENT_POLICY_FIELDS
            or policy.get("schema_version") != 1 or policy.get("adapter_protocol") != "payment-adapter-v1"
...
def run_candidate():
    ...
    return subprocess.run(
        [str(SANDBOX), "--policy", str(SANDBOX_POLICY), "--deny-network",
         "--protocol", "candidate-probe-v1", "--", str(CANDIDATE)], cwd="/",
        env={"PATH": "/usr/bin:/bin", "LANG": "C", "PAYGATE_NETWORK_DENY": "required"},
        input=b"{}", stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
...
def run_adapter(a, key, cap):
    ...
    return subprocess.run(
        [str(ADAPTER), "--protocol", "payment-adapter-v1", "--attempt-key", key,
         "--source-commit", a.source_commit, "--cargo-lock-sha256", a.cargo_lock_sha256,
         "--workflow-run-id", a.workflow_run_id, "--backend", a.backend,
         "--cap-msat", str(cap), "--result", str(RESULT)],
        cwd="/", env={"PATH": "/usr/bin:/bin", "LANG": "C"}, stdin=subprocess.DEVNULL,
        stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=30, check=False,
    )


def validate_local_result(a, key, cap):
    """Only a structural pre-check; the external signed result validator remains authoritative."""
    validate_protected_path(RESULT)
    try:
        result = json.loads(RESULT.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError("malformed trusted adapter result") from exc
    required = {"backend", "source_commit", "cargo_lock_sha256", "workflow_run_id", "attempt_key", "cap_msat", "state"}
    if not isinstance(result, dict) or not required.issubset(result):
        raise ValueError("incomplete trusted adapter result")
    if any(result[name] != value for name, value in {
        "backend": a.backend, "source_commit": a.source_commit,
        "cargo_lock_sha256": a.cargo_lock_sha256, "workflow_run_id": a.workflow_run_id,
        "attempt_key": key, "cap_msat": cap,
    }.items()):
...
def main():
    ...
    a = p.parse_args()
    if (a.result != RESULT or a.backend not in BACKENDS or not COMMIT.fullmatch(a.source_commit)
            or not DIGEST.fullmatch(a.cargo_lock_sha256) or not RUN_ID.fullmatch(a.workflow_run_id)):
    ...
    try:
        ...
        raise SystemExit("protected deployment unavailable; no invoice created") from exc
    key = attempt_key(a)
    try:
    ...
    try:
        ...
        raise SystemExit("candidate unavailable before invoice creation")
    if probe.returncode or probe.stdout != b'{"qualification_request":"candidate-probe-v1"}\n':
        transition({"attempt_key": key, "state": "unsubmitted_released"})
    ...
    try:
        ...
        try:
            transition({"attempt_key": key, "state": "submitted_unknown_permanent_no_retry"})
        except ValueError:
            pass
        raise SystemExit("adapter outcome ambiguous; permanently refusing retry")
