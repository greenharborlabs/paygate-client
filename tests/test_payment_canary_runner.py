...
MODULE = (
    Path(__file__).parents[1] / "infra/payment-canary-runner/payment_canary_runner.py"
)
...
def test_ledger_claim_is_exclusive_and_release_is_never_reclaimable(
    ...
    assert runner.transition(
        {"attempt_key": "attempt", "state": "unsubmitted_released"}
    )
...
def test_signed_candidate_approval_binds_digest_and_rejects_tampering(monkeypatch):
    approval = {
        ...
        "source_commit": "a" * 40,
        "cargo_lock_sha256": "b" * 64,
        "attestation_subject": "sha256:" + "c" * 64,
        "attestation_digest": "c" * 64,
        "approval_id": "approval-1",
        "approved_backends": ["lnd-testnet-canary"],
        "issuer": "issuer",
        "key_id": "key",
    ...
    keyring = {
        "purpose": "candidate-approval",
        "keys": [
            {
                "id": "key",
                "issuer": "issuer",
                "public_key": "/deployment/key.pub",
    ...
    monkeypatch.setattr(
        ...
        lambda path: envelope if path == runner.APPROVAL else keyring,
    )
...
def test_candidate_approval_rejects_unknown_revoked_or_mismatched_signer(
    monkeypatch, key_change
):
    claims = {
        "approved": True,
        "candidate_sha256": "c" * 64,
    ...
    monkeypatch.setattr(
        ...
        lambda *args: pytest.fail("untrusted key reached verifier"),
    )
    with pytest.raises(ValueError):
        runner.validate_candidate_approval(args())


def test_policy_refuses_candidate_authority_and_adapter_uses_literal_context(
    monkeypatch,
):
    unsafe = {
        "schema_version": 1,
    ...
    assert (
        "--cap-msat" in command and command[command.index("--cap-msat") + 1] == "1000"
    )
...
def test_candidate_sandbox_uses_fixed_policy_network_and_protocol(monkeypatch):
    ...
    assert kwargs["env"] == {
        "PATH": "/usr/bin:/bin",
        "LANG": "C",
        "PAYGATE_NETWORK_DENY": "required",
    }


def test_local_result_distinguishes_definite_failure_from_ambiguous_record(
    tmp_path, monkeypatch
):
