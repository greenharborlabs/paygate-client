...
CANONICAL_VECTORS = {
    "lnd-testnet-canary": {
        ...
        "canonical_json": "{\"backend\":\"lnd-testnet-canary\",\"environment_configuration_digest\":\"sha256:3a143b10d8b7cb5b1cf2dd15449b3d83a2cb31dc2f413ac6f373921e63c8752a\",\"profile\":\"paygate-github-protection-v1\",\"repository\":\"greenharborlabs/paygate-client\",\"runner_group_configuration_digest\":\"sha256:99c5e2da02bc57cccedf5677787ad005f8f58c4633eb4d1de3349fd764997d9f\"}",
        "composite_digest": "sha256:11983aac8c2c7b783c90338259ef3cc253854ed32ceb464366e96e1d9bac0ce2",
    },
    "breez-mainnet-canary": {
        "id": "breez-mainnet-canary-redacted-v1",
        "canonical_json": "{\"backend\":\"breez-mainnet-canary\",\"environment_configuration_digest\":\"sha256:cfc6e441afbdbc2bd9f9b4ab9d209c66f4bea0da2390a1e9d4dd723a7ab8cf92\",\"profile\":\"paygate-github-protection-v1\",\"repository\":\"greenharborlabs/paygate-client\",\"runner_group_configuration_digest\":\"sha256:99c5e2da02bc57cccedf5677787ad005f8f58c4633eb4d1de3349fd764997d9f\"}",
        "composite_digest": "sha256:00b3ff268333e8761a179e6d7572a3015c29e46c541e5f21bd2caead9bb28957",
    },
}
FIELDS = {
    "backend",
...
def semantic_object(vector):
    return {
        ...
        "runner_group_configuration_digest": vector["runner_group_configuration_digest"],
    }
...
def test_profile_and_redacted_vectors_are_exact_and_reproducible():
    ...
    assert set(profile) == {
        "canonical_object", "canonicalization", "composite_digest",
        "future_schema_v7_claims", "legacy_or_unknown_profile_behavior",
        "profile", "repository", "status",
    }
    ...
    assert profile["canonical_object"]["backend"] == {"type": "string", "enum": ["lnd-testnet-canary", "breez-mainnet-canary"]}
    assert profile["canonical_object"]["profile"] == {"type": "string", "const": PROFILE_NAME}
    assert profile["canonical_object"]["repository"] == {"type": "string", "const": REPOSITORY}
    for name in ("environment_configuration_digest", "runner_group_configuration_digest"):
        assert profile["canonical_object"][name] == {"type": "string", "pattern": "sha256:<64 lowercase hex>"}
    assert profile["canonicalization"] == {
        "encoding": "utf-8",
        "json": "sorted keys, comma/colon separators, ASCII-safe JSON",
        "object_fields": ["backend", "environment_configuration_digest", "profile", "repository", "runner_group_configuration_digest"],
    }
    assert profile["composite_digest"] == "sha256 of exact canonical object bytes; lowercase sha256:<hex>"
    assert profile["legacy_or_unknown_profile_behavior"] == "fail-closed when schema-v7 activation occurs"
    assert profile["future_schema_v7_claims"] == ["digest_profile", "baseline_composite_digest"]
    ...
    assert len({item["backend"] for item in vectors["vectors"]}) == len(vectors["vectors"])
    assert {item["backend"] for item in vectors["vectors"]} == BACKENDS
    for vector in vectors["vectors"]:
        assert set(vector) == {
            "id", "backend", "environment_configuration_digest",
            "runner_group_configuration_digest", "canonical_json", "composite_digest",
        }
...
def test_order_and_whitespace_are_invariant_but_each_semantic_field_changes_digest():
    ...
    whitespace_variant = json.loads("  " + json.dumps(baseline, indent=2) + "\n", object_pairs_hook=reject_duplicates)
...
@pytest.mark.parametrize(
    "path, raw",
    [
        ("profile", '{"canonical_object":{"backend":{"type":"string","type":"string"}}}'),
        ("vectors", '{"profile":"x","repository":"x","vectors":[{"id":"one","id":"two"}]}'),
        ("vectors", '{"profile":"x","repository":"x","vectors":[{"nested":{"a":1,"a":2}}]}'),
    ],
)
